...
class LinearBt(Callback):
    ...
    def before_fit(self):
        ...
        #     test_eq(self.learn.n_epoch%self.tune_save_after,0) #check that e.g. if we save every 50 epochs, then
        #                                                  # the total number of epochs is 50 or 100, (multiple of 50)
...
class main_train:
    ...
    def __init__(self,
                 ...
                 tune_save_after=50, #save tuned model after every 50 epochs
                ):
        store_attr()
    ...
    def fit(learn,fit_type,epochs,freeze_epochs,initial_weights,pretrain,lr_max=None):
        ...
        if fit_type == 'encoder_fine_tune': #i.e. barlow twins
            ...
            print('about to use `fit` function')
            learn.fit(epochs) #we found that fit_one_cycle didn't work well in this case

        elif fit_type == 'fine_tune': #there are basically 3 cases:

            #First case: random initial weights, and we didn't pretrain
            ...
            #Second case: we are fine tuning a pretrained ImageNet model 
            elif pretrain == False and initial_weights in ['bt_pretrain','supervised_pretrain']:
                learn.freeze()
                print('froze body')
    ...
    def fine_tune(self):
        ...
        learn = Learner(self.dls_tune,model,splitter=my_splitter,
                        cbs = [LinearBt(aug_pipelines=self.aug_pipelines_tune,
                                        n_in=self.n_in,tune_model_path=self.tune_model_path,
                                       tune_save_after=self.tune_save_after)],wd=0.0
        ...
        if self.tune_model_path != None:
            ...
            metrics['tune_paths'] = learn.tune_model_path_dict #paths after 50,100, etc epochs
...
lr_max_rand = 0.00015848931798245758
lr_max_sup = 0.0002290867705596611
